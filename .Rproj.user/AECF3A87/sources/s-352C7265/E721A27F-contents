...
Escolaridade <- factor(df$CS_ESCOL_N,
                       ...
                       labels=c("Analfabeto", "Fundamental I incompleto", "Fundamental I completo",
                                ...
                                "M?dio incompleto", "M?dio completo",
                                "Superior incompleto", "Superior completo", "Ignorado","N?o se aplica"))
...
Gestante<- ifelse(df$CS_GESTANT==9,"Ignorado",
             ifelse(df$CS_GESTANT==6,"NSA",
                    ifelse(df$CS_GESTANT==5,"N?o","Sim")))
...
esc.dt$label <- paste0(esc.dt$resp, "\n ",esc.dt$fraction,"%")

cores <- c("#F7FCB9","#006D2C","#41AB5D","#A50F15")

ggplot(esc.dt, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=Gestante)) +
  geom_rect() +
  geom_text( x=c(3.5,3.5,3.5,4), aes(y=labelPosition, label=label),size=12,col="Black")+
  scale_fill_manual(name = " ",  values =cores)+
...
ggplot(data = d, 
       mapping = aes(
         x = c("1? Trimestre","2? Trimestre","3? Trimestre","Ignorado"), 
         y = Percentuais, 
         fill = factor(q.val),
         label=paste(round(Percentuais*100, 0), "%", sep="")
       )) +
...
#df1 <- as.data.frame(df1[-14,])

#df_teste <- df %>% group_by(DT_NOTIFIC,week(DT_NOTIFIC)) %>% summarise(Freq = n())

#week(df$DT_NOTIFIC)

fig<-plot_ly(x = df2$label_serie, y = df2$frequencia, mode = 'lines+markers', type="scatter")
fig


# , 
# text = "", marker=list(size=10), 
# line = list(color = "#09557f", width = 4), name = 'Suscet?veis')
# fig
# 
# 
